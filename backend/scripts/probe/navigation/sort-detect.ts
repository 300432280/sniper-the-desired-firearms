 ...
 */
// backend/scripts/probe/navigation/sort-detect.ts
// Navigation stage Task 5.2: Detect sort param by reading <select> HTML, filtering newest-style
...
// Celerant / path-form: detect if the testUrl already has /orderby/<value>/ baked
// into the path (Mistake 36). If so, swapping the value in the path is the correct
// way to test a different sort — query params are ignored when path already specifies.
const PATH_ORDERBY_RE = /\/orderby\/[^/?#]+/i;

function hasPathOrderby(url: string): boolean {
  try { return PATH_ORDERBY_RE.test(new URL(url).pathname); }
  catch { return false; }
}

function swapPathOrderby(baseUrl: string, newValue: string): string {
  const u = new URL(baseUrl);
  u.pathname = u.pathname.replace(PATH_ORDERBY_RE, `/orderby/${newValue}`);
  return u.toString();
}

function currentPathOrderby(url: string): string | null {
  try {
    const m = PATH_ORDERBY_RE.exec(new URL(url).pathname);
...
export async function detectSort(state: GeographyCountState): Promise<SortDetectResult> {
  ...
  // 3a. Celerant / path-form sort detection (Mistake 36).
  // If the testUrl already contains /orderby/<value>/ in its path, the site uses
  // path-style sort (Celerant ColdFusion convention). Query-params on top would
  // be ignored. Test by SWAPPING the orderby value in the path.
  if (hasPathOrderby(testUrl)) {
    const currentSort = currentPathOrderby(testUrl);
    process.stderr.write(`  [sort-detect] PATH-FORM detected: testUrl has /orderby/${currentSort}/\n`);

    // Find a counter-control value (alpha/price/popularity) to prove default IS sorted
    const counter = findCounterControl(selects);
    if (counter && counter.value !== currentSort) {
      const counterPathUrl = swapPathOrderby(testUrl, counter.value);
      process.stderr.write(`  [sort-detect] path-form counter-control: ${counterPathUrl}\n`);
      const counterPathPage = await fetchAndExtract(counterPathUrl, state);
      const counterPathFirst3 = first3Urls(counterPathPage.products);

      const verdict = threeOutcomeVerdict(
        defaultFirst3,
        defaultFirst3, // default IS the candidate (path already has newest sort)
        counterPathFirst3,
        state.walkedUniqueCount || defaultPage.products.length,
      );
      process.stderr.write(`  [sort-detect] path-form verdict: ${verdict} (default first=${defaultFirst3[0]?.split('/').pop()}, counter first=${counterPathFirst3[0]?.split('/').pop()})\n`);

      if (verdict === 'honored-default-is-newest') {
        // Path-baked sort verified. Convention: sortParam = "" (empty string),
        // sortVerified inferred true downstream by watermark-method.
        return {
          sortParam: '',
          evidence: {
            selectHtml: `[PATH-FORM /orderby/${currentSort}/] ${selectHtml}`,
            candidateParams: [`/orderby/${currentSort}/ (path-baked)`],
            dateVerification: null,
            idJumpBefore: defaultFirst3[0] || '',
            idJumpAfter: counterPathFirst3[0] || '',
          },
        };
      }
    } else {
      process.stderr.write(`  [sort-detect] path-form: no counter-control candidate available; falling through to query-form tests\n`);
    }
  }
