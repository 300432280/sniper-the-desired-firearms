...
// Step 2 of new Room 3 sequence: multi-source candidate generation.
...
export async function discoverCatalogUrls(
  ...
  const navAnchors = $('nav a, header a, .menu a, [class*="nav"] a, .main-menu a, .mega-menu a')
  ...
  // For BC Stencil: use category tree recursion instead of naive nav probe.
  // BC Stencil mega-menus can have 1000+ links (caliber pages, brand pages, etc.)
  // but only ~20-30 are actual product category leaf nodes. Strategy:
  // 1. Identify top-level category links (1-segment paths in nav)
  // 2. For each, fetch page and find subcategory links
  // 3. Probe subcategories for real products (not sub-cat tiles)
  if (/bigcommerce/.test(state.platform)) {
    const bcCandidates = await discoverBcStencilCategories(state, origin, uniqueNav, existingUrls, ctx);
    if (bcCandidates.length > 0) {
      allCandidates.push(...bcCandidates);
      if (primarySource === 'manual') primarySource = 'category-tree-walk';
      process.stderr.write(`  [catalog-urls] BC Stencil category walk: ${bcCandidates.length} leaf categories\n`);
    }
  } else {
    // Non-BC: probe nav URLs (limit to 30 to avoid hammering)
    const newNavUrls = uniqueNav.filter(u => !existingUrls.has(u.replace(/\/$/, '').toLowerCase()));
    for (const url of newNavUrls.slice(0, 30)) {
      const probed = await probeUrl(url, state, ctx);
      if (probed && probed.page1ProductCount >= 3) {
        allCandidates.push(probed);
      }
